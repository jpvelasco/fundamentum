...
- **Cannot disable tools via `.codacy.yml`.** The `enabled: false` option only works for languages (`languages.<lang>.enabled: false`). Tools can only be enabled/disabled on the [Code patterns page](https://docs.codacy.com/repositories-configure/configuring-code-patterns/) in the Codacy UI. The `tools:` key was from the old Codacy CLI v2 (`.codacy/codacy.yaml`) and is not recognized by current cloud config. Use the maintained npm CLIs (`@codacy/codacy-cloud-cli`, `@codacy/analysis-cli`) via npx for all local/cloud interaction.
