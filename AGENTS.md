...
- **export** — write a portable JSON baseline from the current flags (`--preset`, `--ci`, `--require-checks`, `--advanced-security`, `--strict`)

Shared flags on root: `--dry-run`, `--verbose`, `--token`, `--no-overwrite`, `--pr`, `--advanced-security`, `--strict`, `--require-checks`, `--ci`, `--preset`, `--from`.
`init` also takes `--private` (default `true`; pass `--private=false` for public).
Shared flag `--ci auto|go|node|python|rust|generic|none` selects the starter CI pack (`auto` detects from `go.mod` / `package.json` / `pyproject.toml` / `requirements.txt` / `Cargo.toml`, otherwise generic). Shared flag `--preset oss|private|strict` skips wizard prompts (`strict` also sets `--strict` and `--advanced-security`).
...
- `cmd/exportcmd` — writes a portable JSON baseline for `--from`
- `cmd/repoinit` — creates repo via API, then delegates to apply
- `cmd/globals` — shared mutable flag state (DryRun, Token, Verbose, NoOverwrite, ViaPR, AdvancedSecurity, Strict, RequireChecks, CIPack, Preset, FromFile)
...
- `internal/templatefs` — `//go:embed` of template files; `dotgithub/` maps to `.github/`, `dotcodacy.yml` to `.codacy.yml`; `public_`/`private_` filename prefixes gate by visibility and are stripped from the target; `generic_`/`node_`/`python_`/`rust_` prefixes select the CI pack and are stripped the same way
...
- **Branch protection**: tries modern ruleset first (names `protect-main`, `protect-version-tags`), falls back to classic protection only when the 403 says rulesets are unavailable on this plan (`IsRulesetUnavailable`). Token-scope / SSO / IP-allow-list 403s surface as errors. Existing managed rulesets are fetched and compared, not trusted by name — a disabled or emptied `protect-main` is reconciled in place. **Limitation:** the classic protection API requires GitHub Pro — free-tier private repos must configure branch protection manually via Settings → Branches. Use the `--no-overwrite` flag if you only want to add missing files.
...
- **Core branch protection is required**: `protect-main` create/upgrade is not `Optional`. A failed ruleset (or classic fallback) fails the run and suppresses the Done banner. Tag ruleset and security remain optional (plan-limit / GHAS).
- **Wizard prompts consume exactly one line each** (`readLine` in `internal/wizard/wizard.go`): never swap them back to per-prompt `bufio.Scanner` — Scanner buffers ahead, so piped/scripted stdin loses answers after the first prompt and later prompts silently take defaults (declined items got applied once). Regression tests live in `wizard/piped_input_test.go`. Progress and skip lines take the injected `io.Writer` (`applyItems`, `ApplyViaPR`, `ShouldSkip`, `SelectInteractive`) — never `fmt.Printf` to process stdout.

- **Existence helpers fail closed** (`RulesetExists`, `ClassicProtectionExists`, `AnyFileExists`): only HTTP 404 means missing. Auth, rate-limit, and 5xx responses are errors, not "does not exist" — a 403 must not skip creating `protect-main`. Ruleset listing follows `Link: rel="next"` so a later-page `protect-main` is not treated as missing.
...
- **`init --dry-run`**: plans create+harden without calling `GetRepo`. Files, settings, and rulesets are modeled as absent; visibility comes from `--private`.
- **`--no-overwrite`**: skips any file that already exists, even if content differs
- **`--pr`**: batches file changes into a single PR; non-file items (settings, security, protection) still apply directly. A runtime banner plus `--help` / README spell that split out so operators do not treat `--pr` as files-only. Required status checks (Codacy, shipped CI) are deferred so the open harden PR cannot be deadlocked by contexts that only exist after merge. The same defer happens on 409 auto-fallback to PR mode. Re-apply after the harden PR lands to require those checks. Contents `?ref=` uses `url.Values` so branch names with `+`/`&` round-trip.
- **`--strict`**: optional core steps (tag ruleset, security) fail the run the same way required items do, and suppress the Done banner. Default remains warn-and-continue for those steps.
- **`--require-checks`**: comma-separated required status-check contexts for `protect-main`. Default is the shipped CI jobs that report on PRs, not Codacy (a missing Codacy App deadlocks merges). Empty list (`--require-checks=""`) requires none.
- **Paid GHAS gating** (`cmd/apply/apply.go` `buildItems`): private/internal repos get Dependabot only unless `--advanced-security` is passed or the wizard prompt is answered yes (prompt only when not dry-run). Dry-run still *shows* the offered secret-scanning plan line for that visibility. Public repos always get secret scanning + push protection. Internal visibility uses private templates.
...
- Auth: `--token` flag or `GITHUB_TOKEN` env var, used as Bearer token. `apply`, `audit`, and live `init` fail locally when neither is set. `init --dry-run` does not require a token. `WithBaseURL` allows non-HTTPS only for true loopback (`localhost`, `127.0.0.1`, `::1`), not prefix matches.
