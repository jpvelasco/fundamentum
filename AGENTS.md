...
- `internal/templatefs` — `//go:embed` of template files; `dotgithub/` maps to `.github/`, `public_codacy.yml` to `.codacy.yml`; `public_`/`private_` filename prefixes gate by visibility and are stripped from the target (except `public_codacy.yml`, which keeps its dot-prefixed `.codacy.yml` target); `generic_`/`node_`/`python_`/`rust_` prefixes select the CI pack and are stripped the same way
