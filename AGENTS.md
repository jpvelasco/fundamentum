...
- **Existence helpers fail closed** (`RulesetExists`, `ClassicProtectionExists`, `AnyFileExists`): only HTTP 404 means missing. Auth, rate-limit, and 5xx responses are errors, not "does not exist" — a 403 must not skip creating `protect-main`.
