...
- PR-level Codacy checks come from the GitHub integration webhook — no workflow involved. But that webhook doesn't re-fire on push to `main` after a squash-merge, so this repo's own `.github/workflows/ci.yml` also runs a `Codacy Analysis` job (`push`-only, `continue-on-error: true`) that re-uploads results via `@codacy/codacy-analysis-cli-action`, keeping the dashboard in sync without ever blocking a push. **Do not make it blocking** while the repo is analyzed in Codacy cloud — the CLI's final notification returns `Feature "Repository Analysis" is disabled` and the job would fail red on every main push. Toggle `continue-on-error` back off only if "Run analysis on your build server" is enabled in Codacy settings. This job is this-repo-specific, not part of the shipped templates.
