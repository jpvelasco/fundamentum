...
const REPO = "jpvelasco/fundamentum";
...
function getArchiveName(version, platform, arch) {
  ...
  return `fundamentum_${version}_${os}_${cpu}.${ext}`;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(url, { timeout: DOWNLOAD_TIMEOUT_MS }, (res) => {
      if (
        ...
        if (
          !RELEASE_HOSTS.has(nextUrl.hostname) &&
          !nextUrl.hostname.endsWith(".githubusercontent.com")
        ) {
          return reject(
            new Error(`Refusing redirect to unexpected host: ${nextUrl.hostname}`)
          );
        }
        res.resume();
...
function verifyChecksum(buffer, archiveName, pkg) {
  ...
  console.log("fundamentum: checksum verified (SHA-256)");
}
...
async function main() {
  ...
  console.log(`fundamentum: downloading ${archiveName}...`);
  const buffer = await download(url);

  verifyChecksum(buffer, archiveName, pkg);

  console.log("fundamentum: extracting binary...");
